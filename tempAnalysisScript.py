...
import matplotlib            #For matplotlib in general.
matplotlib.use('Agg');       #For Non-interactive backend (Non-GUI).
import pylab;                #For matrices.
import matplotlib.pyplot;    #For plots.
import matplotlib.animation; #For animating plots.

#============================================================#
#Delimiter Characters 
prefix_temperatures  = "\[RMD_TRACE_TEMPERATURE\]" #prefix delimiter.
#============================================================#
...
#Diagram Title base.
title = "Heat Distribution on the Block Cycle = %d";
...
#Min-max temperature and degree steps.
min_temperature, max_temperature = (0, 200);
step = 1;
temperature_units = "Celsius";

# Setting up a colormap.
#mycmap = mpl.colors.LinearSegmentedColormap.from_list('mycolors',['yellow','red']);
mycmap = matplotlib.pyplot.cm.jet;
...
#{
    ...
    #{
        for string in re.findall(prefix_temperatures+" Temperature (.*)", line): #iterate each encapsulated string in line.
            tuple = re.findall(r'\d+[.]*\d*', string);
            temperature = float(tuple[0]); cycle = int(tuple[1]);
...
for arg in sys.argv:
#{
    parse_dimensions(arg);
#}

if (blk_width == 0 or blk_height == 0):
#{
    print "Error: invalid block dimensions of {} by {} specified!".format(blk_width, blk_height);
    exit(0);
#}
if (unt_width == 0 or unt_height == 0):
#{
    print "Error: invalid unit dimensions of {} by {} specified!".format(unt_width, unt_height);
    exit(0);
#}

print "Block layout: {} by {}...".format(blk_width, blk_height);
print "Unit layout: {} by {}...".format(unt_width, unt_height);
...
print "Time to generate graph : {:02}:{:02}:{:02}...".format(t[0], t[1], t[2]);
#============================================================#
